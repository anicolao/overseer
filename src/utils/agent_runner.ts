...
export class AgentRunner {
	...
	private validateDoneResponse(
		...
		if (!state.persistSucceededAfterWrite) {
			return 'task_status "done" is not allowed after a successful repository write action until your changes are successfully persisted';
		}
	...
	private buildProgressReminder(
		...
		if (!state.persistSucceededAfterWrite) {
			return "You have already modified repository files in this task. Do not finish until your changes are successfully persisted.";
		}
		if (requirePostPersistVerification && !state.verifiedAfterPersist) {
