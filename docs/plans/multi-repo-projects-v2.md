...
## Increment 1: App Authentication Foundation
...
## Increment 2: Projects V2 GraphQL Methods
...
## Increment 3: Dispatcher Event Handling
**Goal**: Make the dispatcher capable of processing `projects_v2_item` events.
- **Files**: `src/dispatch.ts`
- **Actions**:
  1. Update `run()` in `src/dispatch.ts` to process `eventName === "projects_v2_item"`.
  2. Map the Single Select field changes to internal persona handles (e.g., Triage -> `@overseer`) and extract the repository/issue information from the project item.
- **Verification**: Ensure the event handler properly parses a mocked `projects_v2_item` payload.
